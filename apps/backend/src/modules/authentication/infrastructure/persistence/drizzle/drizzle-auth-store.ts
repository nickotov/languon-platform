...
function passkeyInsertValues(passkey: NewStoredAuthenticationPasskey) {
  return {
    ...
    publicKey: Buffer.from(passkey.credentialPublicKey).toString("base64url"),
      ...
      return {
        decision: {
          ...
          retryAfterSeconds: Math.max(
            1,
            Math.ceil((cooldownEndsAt - challenge.issuedAt.getTime()) / 1_000),
          ),
        },
        previous: active,
        rolling: [],
...
function rollingSendWindow(
  ...
  >(
    (candidate, row) =>
      !candidate || row.lastSentAt < candidate.lastSentAt ? row : candidate,
    null,
  );
  return {
...
export class DrizzleAuthStore implements AuthStore {
  ...
  public registerOrReplacePendingSignup(input: {
    ...
    return this.database.transaction(async (transaction) => {
      ...
      if (email.canonicalValue !== input.account.canonicalEmail) {
        throw new Error("The signup email canonical value is inconsistent.");
      }
  ...
  public replaceEmailVerificationChallenge(input: {
    ...
    return this.database.transaction(async (transaction) => {
      ...
      if (!account || account.status !== "pending") return { kind: "noop" };
      const assessment = await assessChallengeIssue(
        transaction,
        input.challenge,
      );
  ...
  public commitEmailVerification(input: {
    ...
    return this.database.transaction(async (transaction) => {
      const challenge = await findChallengeForUpdate(transaction, input.flowId);
      if (
        !challenge ||
      ...
      assertInitialSession(input.session, "email_verification", input.now);
      ...
      if (input.session.userId !== account.userId) {
        throw new Error("The verification session belongs to another user.");
      }
      await transaction
        .update(authVerificationChallengesTable)
  ...
  public commitPasswordReset(input: {
    ...
    return this.database.transaction(async (transaction) => {
      ...
      if (input.newCredential.userId !== account.userId) {
        throw new Error("The password credential belongs to another user.");
      }
      await transaction
        .update(authVerificationChallengesTable)
        .set({ consumedAt: input.now, updatedAt: input.now })
  ...
  public issueSessionForActiveUser(input: {
    ...
    if (input.replacementCredential && !input.expectedPasswordHash) {
      throw new Error("Credential replacement requires the expected hash.");
    }
    return this.database.transaction(async (transaction) => {
      ...
      if (input.replacementCredential) {
        ...
        await savePasswordCredential(transaction, input.replacementCredential);
      }
      await insertSession(transaction, input.session);
  ...
  public changePasswordAndReplaceSessions(input: {
    ...
    assertInitialSession(input.replacementSession, "password", input.revokedAt);
    return this.database.transaction(async (transaction) => {
      await lockUser(transaction, input.userId);
      await lockPasswordCredential(transaction, input.userId);
      const account = await findAccount(
  ...
  public rotateRefreshSession(input: {
    ...
    return this.database.transaction(async (transaction) => {
      ...
      if (
        ...
        input.successor.authenticationMethod !== current.authenticationMethod ||
        ...
        throw new Error("The refresh successor does not preserve its family.");
      }
  ...
  public registerPasskey(input: {
    ...
    if (input.passkey.revokedAt !== null || input.passkey.lastUsedAt !== null) {
    ...
    return this.database.transaction(async (transaction) => {
      ...
      if (activePasskeys.length >= maximumActivePasskeysPerUser) return false;
      try {
        await transaction
