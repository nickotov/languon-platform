...
export class SimpleWebAuthnAdapter implements PasskeyVerifier {
  ...
  public constructor(options: SimpleWebAuthnAdapterOptions) {
    if (
      ...
      !options.origins.every((origin) => isExactOrigin(origin, options.rpId))
    ) {
      throw new RangeError("The WebAuthn relying-party settings are invalid.");
    }
    ...
    try {
      ...
      const result = await verifyAuthenticationResponse({
        credential: {
          ...
          publicKey: new Uint8Array(input.storedCredential.credentialPublicKey),
  ...
  public async verifyRegistration(input: {
    ...
    try {
      assertMatchingCredentialIds(input.credential.id, input.credential.rawId);
...
function decodeBase64Url(value: string): Uint8Array {
  if (value.length < 16 || value.length > 128 || !isCanonicalBase64Url(value)) {
