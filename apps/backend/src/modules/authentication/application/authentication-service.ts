...
export class AuthenticationService {
  ...
  public capabilities(): {
    ...
    const emailAvailable = this.dependencies.emailSender.capability().available;
    return {
      email: {
  ...
  public async signUp(input: {
    ...
    await this.limit(input.context, [
      ["verification.email_send.cooldown", email.canonicalValue, 1, 60_000],
      [
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      const emailId = existing?.emailId ?? this.dependencies.ids.generate();
      ...
      const result =
        await this.dependencies.store.registerOrReplacePendingSignup({
          ...
          credential: this.createCredential(userId, passwordHash, now),
        });
  ...
  public async verifyEmail(input: {
    ...
    return this.dependencies.sessionIssuer.toSuccess(prepared, result.account);
  }
  ...
  public async passwordLogin(input: {
    ...
    const email = EmailAddress.create(input.email);
    await this.limit(input.context, [
      ["login.password.account", email.canonicalValue, 10, 15 * 60 * 1_000],
      [
    ...
    const account = await this.dependencies.store.findAccountByCanonicalEmail(
      email.canonicalValue,
    );
    const candidateHash =
    ...
    if (committed.kind !== "issued" && replacementCredential) {
      ...
      if (
        ...
        if (currentVerification.matches) {
          committed = await this.dependencies.store.issueSessionForActiveUser({
            expectedPasswordHash: current.passwordCredential.encoded,
            session: prepared.storedSession,
  ...
  public async forgotPassword(input: {
    ...
    await this.limit(input.context, [
      ...
      ["password.forgot.account", email.canonicalValue, 5, 60 * 60 * 1_000],
      [
    ...
    if (!account || account.status !== "active" || !account.emailVerifiedAt) {
      return { recovery: this.syntheticFlow(now), status: "recovery_pending" };
    }
    ...
    const result = await this.dependencies.store.issuePasswordResetChallenge({
      challenge: issued.challenge,
  ...
  public async resetPassword(input: {
    ...
    const result = await this.dependencies.store.commitPasswordReset({
      ...
      flowId: challenge.flowId,
      newCredential: this.createCredential(challenge.userId, passwordHash, now),
      now,
    });
    if (result !== "reset") {
      await this.recordFailure(
  ...
  public async changePassword(input: {
    ...
    const prepared = await this.dependencies.sessionIssuer.prepareReplacement({
      ...(input.clientLabel === undefined
        ? {}
        : { clientLabel: input.clientLabel }),
      method: "password",
  ...
  public async refresh(input: {
    ...
    try {
      prepared =
        await this.dependencies.sessionIssuer.prepareSuccessor(predecessor);
    } catch (error) {
  ...
  public async requireActiveSession(input: {
    ...
    session: NonNullable<Awaited<ReturnType<AuthStore["getActiveSession"]>>>;
  }> {
  ...
  public async finishPasskeyRegistration(input: {
    ...
    try {
      await this.limit(input.context, [
        ["passkey.registration.finish.user", input.userId, 10, 15 * 60_000],
        [
      ...
      const challenge = await this.dependencies.webAuthnChallenges.consume(
        input.flowId,
        this.operationOptions(input.context),
      );
      if (
      ...
      const passkey: StoredAuthenticationPasskey = {
        ...
        userHandle: Buffer.from(input.userId, "utf8").toString("base64url"),
        userId: input.userId,
      };
  ...
  public async finishPasskeyAuthentication(input: {
    ...
    try {
      await this.limit(input.context, [
        ["passkey.authentication.finish.flow", input.flowId, 5, 5 * 60_000],
        [
      ...
      const passkey = await this.dependencies.store.findPasskeyByCredentialId(
        input.credential.id,
      );
      ...
      const result = await this.dependencies.store.commitPasskeyAuthentication({
  ...
  private async record(input: {
    ...
    await this.dependencies.securityEvents.record(event).catch(() => undefined);
  }
