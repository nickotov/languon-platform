...
export class SessionIssuer {
    ...
    public constructor(
        ...
        assertPositiveDuration(
            configuration.refreshTokenTtlMs,
            'refresh token',
        );
    ...
    public toSuccess(
        ...
        if (account.status !== 'active' || !account.emailVerifiedAt) {
            throw new TypeError(
                'Only an active verified account can authenticate.',
            );
        }
    ...
    private async prepare(input: {
        ...
        if (input.absoluteExpiresAt <= now) {
            throw new RangeError(
                'A session cannot be issued after its expiry.',
            );
        }
...
function assertPositiveDuration(value: number, label: string): void {
    if (!Number.isSafeInteger(value) || value <= 0) {
        throw new RangeError(
            `The ${label} lifetime must be a positive integer.`,
        );
    }
}
