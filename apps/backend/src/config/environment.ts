...
const RawEnvironmentSchema = z
  ...
  .superRefine((environment, context) => {
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        message: "Production authentication secrets cannot be placeholders.",
        path: ["AUTH_JWT_SECRET"],
      });
    }

    const unsafeFixedCodeSetting = environment.AUTH_ALLOW_INSECURE_FIXED_CODE;
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        message: "Fixed verification codes cannot be enabled in production.",
...
export function loadEnvironment(
  ...
  if (
    !allowedOrigins.every((origin) => {
      ...
      return hostname === webAuthnRpId || hostname.endsWith(`.${webAuthnRpId}`);
    })
  ) {
    throw configurationError(
      "AUTH_WEBAUTHN_RP_ID",
  ...
  return {
    ...
    AUTH_EMAIL_DELIVERY_MODE: fixedCodeEnabled ? "development" : "unavailable",
...
function parseAllowedOrigins(
  ...
  if (new Set(origins).size !== origins.length) {
    throw configurationError("AUTH_ALLOWED_ORIGINS", "Origins must be unique.");
  }
  return origins;
}
...
function parseDurationSeconds(value: string, path: string): number {
  ...
  if (!Number.isSafeInteger(seconds)) {
    throw configurationError(path, "Duration is outside the supported range.");
  }
