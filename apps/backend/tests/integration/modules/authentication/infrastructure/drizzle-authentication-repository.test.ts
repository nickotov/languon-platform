...
function createSession(input: {
    ...
    return {
        ...
        refreshDigest:
            input.refreshDigest ??
            randomUUID().replaceAll('-', '').padEnd(43, 'x'),
...
describe.runIf(isDatabaseIntegrationEnabled())(
    ...
    () => {
        beforeAll(async () => {
            ...
            authenticationUnitOfWork = new DrizzleAuthenticationUnitOfWork(
                database,
            );
      ...
      `;
        ...
        it('keeps one active challenge per user and purpose while preserving purpose separation', async () => {
            ...
            await authenticationUnitOfWork.execute(
        ...
        it("cannot bind a challenge to another user's email", async () => {
            const firstIdentity = await createIdentity(
                'challenge-one@example.com',
            );
            const secondIdentity = await createIdentity(
                'challenge-two@example.com',
            );
        ...
        it('serializes refresh rotation and retains predecessor digests for replay response', async () => {
            ...
            expect(
                generations.find(({ id }) => id === initial.id),
            ).toMatchObject({
            ...
            expect(revoked.every(({ revokedAt }) => revokedAt !== null)).toBe(
                true,
            );
        });
        ...
        it('enforces global passkey credentials, per-user names, and atomic counter updates', async () => {
            const firstIdentity = await createIdentity(
                'passkey-one@example.com',
            );
            const secondIdentity = await createIdentity(
                'passkey-two@example.com',
            );
            ...
            await authenticationUnitOfWork.execute(
                async ({ authentication }) => {
                    const stored =
                        await authentication.findPasskeyByCredentialIdForUpdate(
        ...
        it('atomically serializes and replaces concurrent pending signup state', async () => {
            ...
            const inputs = (['a', 'b'] as const).map((marker) => {
                ...
                return {
                    ...
                    credential: createAuthStorePasswordCredential(
                        userId,
                        marker,
                    ),
                    marker,
            ...
            const signupResults = await Promise.all(
                inputs.map((input) =>
                    authStore.registerOrReplacePendingSignup(input),
                ),
            );
            ...
            const challenge = account
                ...
                : null;
            const marker = challenge?.codeDigest.at(-1) as
                'a' | 'b' | undefined;
            ...
            expect(account?.passwordCredential?.encoded).toBe(
                createAuthStorePasswordCredential(account.userId, marker)
                    .encoded,
            );
        ...
        it('serializes stale-token logout with refresh rotation', async () => {
            ...
            expect(family.every(({ revokedAt }) => revokedAt !== null)).toBe(
                true,
            );
            expect(
                family.every(
