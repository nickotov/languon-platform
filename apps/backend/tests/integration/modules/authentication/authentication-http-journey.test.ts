...
describeIfInfrastructure("composed authentication HTTP journey", () => {
  ...
  beforeAll(async () => {
    ...
    authentication = await createAuthenticationComposition(
      loadEnvironment({
        ...
        AUTH_CODE_HMAC_SECRET: "http-journey-code-secret-40-bytes-aaaaaaaa",
  ...
  it("signs up, verifies, refreshes, signs out, resets, and signs in again", async () => {
    ...
    const currentUser = await get("/users/me", verificationBody.accessToken);
    ...
    expect(
      PasskeyRegistrationOptionsResponseSchema.parse(registrationOptions.body)
        .options,
    ).toMatchObject({
      rp: { id: "localhost" },
    });

    const refreshed = await post("/auth/refresh", {}, { cookie: firstCookie });
    expect(refreshed.response.status).toBe(200);
    const rotatedCookie = cookieFrom(refreshed.response);
    expect(rotatedCookie).not.toBe(firstCookie);

    const logout = await post("/auth/logout", {}, { cookie: rotatedCookie });
    ...
    expect(AuthErrorResponseSchema.parse(rejectedRefresh.body)).toMatchObject({
  ...
  function cookieFrom(response: Response): string {
    ...
    if (!setCookie) throw new Error("Expected the response to set a cookie.");
