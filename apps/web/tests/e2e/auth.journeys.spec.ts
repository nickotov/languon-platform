...
test.describe("authentication journeys", () => {
  ...
  test("password reset revokes old credentials and sessions", async ({
    ...
    try {
      ...
      await expect(secondPage).toHaveURL(/\/login\?returnTo=%2Fsecurity$/);
      ...
      await secondPage
        ...
        .click();
      await expect(secondPage).toHaveURL(/\/security$/);
      await expect(secondPage.locator("dd", { hasText: email })).toBeVisible();
      await secondPage.getByRole("button", { name: "Sign out here" }).click();
      await expect(secondPage).toHaveURL(/\/login$/);
  ...
  test("passkey enrollment, failed assertion, login, rename, and removal", async ({
    ...
    await page.getByRole("button", { name: `Rename ${passkeyName}` }).click();
    ...
    await page
      ...
      .click();
    await page
      .getByRole("button", { name: `Confirm remove ${renamedPasskeyName}` })
      .click();
