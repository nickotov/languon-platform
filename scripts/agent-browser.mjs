...
function validateViewport(values) {
  ...
  if (
    values[0] !== "viewport" ||
    !values.slice(1).every((value) => /^\d+$/.test(value) && Number(value) > 0)
  ) {
...
function validateCommand(command, values) {
  ...
  switch (command) {
    ...
    case "wait":
      ...
      return requireCount(command, values, 1);
    case "screenshot":
      if (!values.every((value) => ["--full", "--annotate"].includes(value))) {
  ...
  return Object.fromEntries(
    Object.entries(environment).filter(([key]) => {
      ...
      return (
        ...
        !["ALL_PROXY", "HTTP_PROXY", "HTTPS_PROXY", "NO_PROXY"].includes(
          normalizedKey,
        )
      );
    }),
